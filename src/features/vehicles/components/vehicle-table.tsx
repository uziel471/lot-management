...
export function VehicleTable({
  ...
  }, [vehicles, statusId, makeId, dateFrom, dateTo])
  ...
  const columns: DataTableColumn<VehicleListItemDTO>[] = [
    ...
    { key: "description", label: "Vehículo", render: (vehicle) => vehicle.description },
    {
      key: "status",
      label: "Estatus",
      render: (vehicle) => <Badge>{vehicle.statusName}</Badge>,
    },
    {
      key: "vin",
      label: "VIN",
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-end gap-3 rounded-lg border p-3">
        ...
        {(statusId || makeId || dateFrom || dateTo) && (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
      ...
      <DataTable
        ...
        getSearchText={(vehicle) =>
          [vehicle.code, vehicle.vin, vehicle.stockNumber, vehicle.description]
            .filter(Boolean)
            .join(" ")
        }
        filterPlaceholder="Buscar por código, VIN o número de inventario…"
